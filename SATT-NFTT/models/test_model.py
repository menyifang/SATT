...
class TestModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.input_Att = self.Tensor(opt.batchSize, 1, opt.fineSize, opt.fineSize)

        if self.opt.use_attimg:
            opt.input_nc = opt.input_nc+3
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.n_downsampling_G,
    ...
    def set_input(self, input):
        # need to use single_dataset mode
        input_A = input['A']
        input_Att = input['Att']
