...
class TextureTransfer(BaseModel):
    def name(self):
    ...
    def initialize(self, opt):
        ...
        self.input_B = self.Tensor(opt.batchSize, opt.output_nc,
                                   opt.fineSize, opt.fineSize)
        self.input_Att = self.Tensor(opt.batchSize, 1,
                                   opt.fineSize, opt.fineSize)
        self.rs = opt.fineSize/2
        ...
        if self.opt.use_attimg:
            opt.input_nc = opt.input_nc+3
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.n_downsampling_G, opt.ngf,
        ...
        if self.isTrain:
            ...
            if self.opt.use_local:
                self.netD2 = networks.define_D(opt.output_nc, opt.ndf*2,
                                              opt.which_model_netD2,
                                              opt.n_layers_D2, opt.norm, use_sigmoid, self.gpu_ids)
        ...
        if not self.isTrain or opt.continue_train:
            ...
            opt.epoch_count = opt.epoch_count + int(opt.which_epoch)
    ...
    def set_input(self, input):
        ...
        self.input_Att.resize_(input_Att.size()).copy_(input_Att)
        self.image_paths = input['A_paths' if AtoB else 'B_paths']
        self.start_points= input['A_start_point']
        self.sal_region = input['sal_region']


    def forward(self):
        self.real_A = Variable(self.input_A)
        self.real_Att = Variable(self.input_Att)
        self.fake_B = self.netG.forward(self.real_A, self.real_Att)
        self.real_B = Variable(self.input_B)

    # no backprop gradients
    def test(self):
        self.real_A = Variable(self.input_A, volatile=True)
        self.real_Att = Variable(self.input_Att, volatile=True)
        self.fake_B = self.netG.forward(self.real_A, self.real_Att)
    ...
    def backward_D2(self):
        # Fake
        # stop backprop to the generator by detaching fake_B
        fake_AB2 = self.fake_AB_pool.query(self.fake_B.clone())
        fake_AB2 = self.crop(fake_AB2, self.rs)

        self.pred_fake2 = self.netD2.forward(fake_AB2.detach())
        self.loss_D2_fake = self.criterionGAN(self.pred_fake2, False)

        # Real
        real_AB2 = self.real_B.clone()
        real_AB2 = self.crop(real_AB2, self.rs)
        self.pred_real2 = self.netD2.forward(real_AB2)
    ...
    def backward_G(self):
        ...
        if self.opt.use_local:
            fake_AB2 = self.crop(fake_AB, self.rs)
            pred_fake2 = self.netD2.forward(fake_AB2)
    ...
    def crop(self, x, rs):
        # crop a patch from image(x) with size(rs)
        salh = self.sal_region[0][0][0][0]
        salw = self.sal_region[0][1][0][0]
        return x[:, :, salh: salh + rs, salw: salw + rs]
