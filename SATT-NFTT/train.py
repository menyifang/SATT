...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset):
        if epoch == 1 or (opt.continue_train and epoch-int(opt.which_epoch)==1) :
            save_data = data
        else:
            save_data = save_data
        ...
        opt.train_G_every_n_iterations = 1
        train_generator = ((epoch + 1) % opt.train_G_every_n_iterations == 0)
